...
def segment(img,detection_result):
    ...
    # 2. 先将整个图片填充为背景色
    masked_array = np.full_like(img_array, [bg_r, bg_g, bg_b])
    ...
    # 将原图的plot区域复制到结果中
    masked_array[y1:y2+1, x1:x2+1] = img_array[y1:y2+1, x1:x2+1]
    
    # 4. 覆盖图例区域,得到一张只有绘图区域的图
    if 'area' in detection_result.legends and detection_result.legends.area.size > 0:
